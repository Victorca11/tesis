...
export default function RawMaterialInfo(props) {
  ...
  async function getRawMaterialData() {
    ...
    let display = <div>
      ...
      <p>Product Repair center: {data[3]}</p>
      <p>Product Transporter: {data[4]}</p>
      <p>Product Mining: {data[5]}</p>
  ...
  function getSupplyChainSteps() {
    return ['At Repair center', 'Collected by Transporter', 'Delivered to Mining'];
  }

  function getSupplyChainStepContent(stepIndex) {
    switch (stepIndex) {
      case 0:
        return 'Raw Material is at Repair center stage in the supply chain.';
      case 1:
        return 'Raw Material collected by the Transporter is on its way to the Mining.';
      case 2:
        return 'Raw Material currently with the Mining';
      default:
